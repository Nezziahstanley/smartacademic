// ============================================================
// SMARTACADEMIC — Federal Polytechnic, Ugep Seed
// Creates departments (schools), programmes, and courses for
// all ND and HND programmes.
// Usage: node scripts/seed-ugep.js
...
/* ============================================================
   COURSES — keyed by programme code
   Each course: [code, title, units, level, semester]
   ============================================================ */
const COURSES = {
...
(async () => {
  try {
    console.log('\n=== Seeding Federal Polytechnic, Ugep ===\n');

    // 1. Departments
    console.log('1. Departments...');
    const deptMap = {};
    for (const d of DEPARTMENTS) {
      const existing = await db.query('SELECT id FROM departments WHERE code = $1', [d.code]);
      if (existing.rows[0]) {
        deptMap[d.code] = existing.rows[0].id;
        console.log(`   ✓ ${d.code} (exists)`);
      } else {
    ...
    for (const p of PROGRAMMES) {
      const existing = await db.query('SELECT id FROM programmes WHERE code = $1', [p.code]);
      if (existing.rows[0]) {
        progMap[p.code] = existing.rows[0].id;
        console.log(`   ✓ ${p.code} (exists)`);
      } else {
        const r = await db.query(
    ...
    for (const [progCode, list] of Object.entries(COURSES)) {
      ...
      if (!progId) {
        console.log(`   ⚠️  Programme ${progCode} not found, skipping courses`);
        continue;
      }
      ...
      for (const [code, title, units, level, semester] of list) {
        const existing = await db.query('SELECT 1 FROM courses WHERE code = $1', [code]);
        if (existing.rows[0]) {
          skipped++;
          continue;
        }
      ...
      console.log(`   + ${progCode}: ${list.length} courses`);
    }

    console.log(`\n✅ Total: ${added} courses added, ${skipped} skipped (already existed)`);

    // Summary
    const summary = await db.query(`
      SELECT d.code AS department, p.code AS programme, c.level, c.semester_name,
             COUNT(*)::int AS courses,
             SUM(c.units)::int AS units
        FROM courses c
        JOIN departments d ON d.id = c.department_id
        JOIN programmes p ON p.id = c.programme_id
       WHERE d.code IN ('CEN','CIV','EEE','CSC','STA','AIT','ARC','BAM','PAD','ACC','LIS','HTM')
       GROUP BY d.code, p.code, c.level, c.semester_name
       ORDER BY d.code, c.level, c.semester_name
    `);
    console.log('\n📊 Summary by Programme/Level/Semester:');
    console.table(summary.rows);

    const totalCourses = await db.query('SELECT COUNT(*)::int AS n FROM courses WHERE is_active = TRUE');
    console.log(`\n🎓 Total active courses in system: ${totalCourses.rows[0].n}`);
  } catch (err) {
