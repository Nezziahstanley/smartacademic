      ...
      if (upd.rowCount > 0) {
        ...
        const students = await db.query(`
          SELECT DISTINCT u.id
            ...
            JOIN students s ON s.id = r.student_id
            JOIN users u ON u.id = s.user_id
           WHERE r.course_id = module.exports = { AND r.session_id = $2 AND r.semester_id = $3
        `, [courseId, session_id, semester_id]);
        const course = await db.query('SELECT code, title FROM courses WHERE id = module.exports = {', [courseId]);

        for (const st of students.rows) {
          await db.query(`
            INSERT INTO notifications (user_id, title, message, type)
            VALUES (module.exports = {, 'Result published', $2, 'result')
          `, [st.id, `Your result for ${course.rows[0].code} — ${course.rows[0].title} is now available.`]);
        }
      }
    }
