...
async function approveResultsBulk(req, res, next) {
  try {
    ...
    for (const cid of course_ids) {
      ...
      const owns = await db.query(
        'SELECT 1 FROM courses WHERE id = module.exports = { AND department_id = $2',
        [courseId, deptId]
      );
      ...
      const upd = await db.query(`
        UPDATE results
           SET submission_status = 'approved',
               approved_at = NOW(),
               approved_by = $4
         WHERE course_id = module.exports = {
           AND session_id = $2
           AND semester_id = $3
           AND submission_status = 'submitted'
        RETURNING id
      ...
      if (upd.rowCount > 0) {
        ...
        const lecturer = await db.query(`
          SELECT l.user_id FROM lecturers l
           WHERE l.id = (SELECT lecturer_id FROM courses WHERE id = module.exports = {)
        `, [courseId]);
        const course = await db.query('SELECT code FROM courses WHERE id = module.exports = {', [courseId]);

        if (lecturer.rows[0] && lecturer.rows[0].user_id) {
          await db.query(`
            INSERT INTO notifications (user_id, title, message, type)
            VALUES (module.exports = {, 'Results approved', $2, 'system')
    ...
    for (const cid of course_ids) {
      ...
      if (upd.rowCount > 0) {
        ...
        if (lecturer.rows[0] && lecturer.rows[0].user_id) {
          await db.query(`
            INSERT INTO notifications (user_id, title, message, type)
            VALUES (module.exports = {, 'Results returned for correction', $2, 'system')
          `, [lecturer.rows[0].user_id, `Your results for ${course.rows[0].code} were returned: ${reason}`]);
        }
      }
    }
