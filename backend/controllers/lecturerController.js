...
async function submitResultsBulk(req, res, next) {
  try {
    ...
    // Notify HOD once
    if (totalSubmitted > 0) {
      const hod = await db.query('SELECT hod_id FROM departments WHERE id = module.exports = {', [department_id]);
      if (hod.rows[0] && hod.rows[0].hod_id) {
        await db.query(`
          INSERT INTO notifications (user_id, title, message, type)
          VALUES (module.exports = {, 'Results submitted for review', $2, 'system')
        `, [
          hod.rows[0].hod_id,
